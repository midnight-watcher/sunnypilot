...
void fill_panda_state(cereal::PandaState::Builder &ps, cereal::PandaState::PandaType hw_type, const health_t &health, bool ignition_source) {
  ...
  ps.setIgnitionSource(ignition_source);
...
std::optional<bool> send_panda_states(PubMaster *pm, const std::vector<Panda *> &pandas, bool is_onroad, bool spoofing_started, bool always_offroad) {
  ...
  // Track previous ignitionCan state for priority (per panda)
  static std::map<std::string, bool> prev_ignition_can_map;
  // Track which ignition source has priority (per panda)
  static std::map<std::string, bool> ignition_source_map;
  ...
  for (const auto& panda : pandas){
    ...
    if (prev_ignition_can_map.find(panda_serial) == prev_ignition_can_map.end()) {
      ...
      ignition_source_map[panda_serial] = false;
    }

    bool prev_ignition_can = prev_ignition_can_map[panda_serial];
    bool& ignition_source = ignition_source_map[panda_serial];

    // Check if ignitionCan should take priority
    if (current_ignition_can) {
      ...
      ignition_source = true;
    } else if (!ignition_source && current_ignition_line) {
  ...
  for (uint32_t i = 0; i < pandas_cnt; i++) {
    ...
    auto ps = pss[i];
    fill_panda_state(ps, panda->hw_type, health, ignition_source_map[panda->hw_serial()]);
